...
TopSongs <- TopSongs %>%
  ...
  mutate(Artist = tolower(gsub("\\s+", "", Artist))) %>%
  mutate(Title = (gsub("[\\(\\)\\-\\.\\,\\?]","", Title)))
```
...
scrape_lyrics <- function(df) {
  ...
  for (i in 1:nrow(df)) {
    url <- paste0("https://www.azlyrics.com/lyrics/", df$Artist[i], "/", df$Title[i], ".html")
    tryCatch({
      html <- read_html(url)
      raw_lyrics <- html_nodes(html, "br+ div") %>% html_text() %>% as.character()
