...
ave_title_repeats<-ggplot(total_repeats_by_year, aes(x = Year, y = AveTotalRepeats)) +
  ...
  theme_minimal() +
  scale_y_continuous(limits = c(0, NA), breaks = seq(0, max(total_repeats_by_year$AveTotalRepeats), by = 5)) + theme(plot.title = element_text(hjust = 0.5)) 
ave_title_repeats
...
ThemeGraph(TopRapSongsCSV)
ThemeGraph(TopCountry)
```
...
```{r}
# TESSA - UNIQUE RATIO FUNCTION WITH GRAPH
library(rvest)
library(stringr)
library(tidytext)
library(dplyr)
...
# generate_unique_ratios(LyricsTopRapSongs3)
...
unique_ratio_graph_by_year <-ggplot(unique_words_ratios, aes(x = Year, y = ratio)) +
  ...
  annotate("text", x = 1988.5, y = overall_unique_words_ratio + 0.01, label = "Average Unique Words Ratio", color = "skyblue3" ) +  # add label to represent what hline means
  labs(title = "Unique Words Ratio of Top Country Songs Over Time",
       x = "Year",
...
```{r}
# TESSA - TITLE REPETITION FUNCTION
...
####JULIA - ADD BETTER GRAPH AND CODE FOR TITLE REPETITON:

#create table with average title repeats within each song per year
ave_title_repeats_by_year<- title_repetition_df_TopCountry %>%
  group_by(Year) %>%
  summarize(Year, AveTitleRepeats=ave(repeats)) %>%
  unique()


# Julia:   
#plot data by year
#include linear model of relationship between year and average repetition
ave_title_repeats<-ggplot(ave_title_repeats_by_year, aes(x = Year, y = AveTitleRepeats)) +
  geom_point( size = 1.2) +
  geom_smooth(method = "lm",se = FALSE, color="skyblue3") +
  labs(title = "Average Title Repeats Over Time",
       x = "Year",
       y = "Mean Title Repeats Per Year") +
  theme_minimal() + scale_y_continuous(limits = c(0, NA), breaks = seq(0, max(ave_title_repeats_by_year$AveTitleRepeats), by = 5)) + theme(plot.title = element_text(hjust = 0.5)) 
ave_title_repeats


#inspect linear model and observe summary statistics
