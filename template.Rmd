...
common_words <- function(song_lyrics) {
  # initialize empty vector to hold words
  all_words <- c() 
  stop_words <-stopwords("en") # initialize English stop words from stopswords package
  
  for (lyrics in song_lyrics) {
    # words <- unlist(strsplit(lyrics, "\\W+")) 
    words <- unlist(strsplit(lyrics, "\\s+")) #splits word at white space
  ...
  unique_df <- unique_df %>% arrange(desc(Frequency))
  return(unique_df)
}

result <-common_words(TopCountry$lyrics)
...
unique_words_ratio_by_year <- function(song_df) {
  ...
  token_lyrics <- song_df %>%
    unnest_tokens(words, lyrics) %>%
    mutate(Year = Year)  # Assuming there's a column 'Release_Date' containing the release date of the songs
  
  # Group by year and calculate unique words ratio
  unique_words_by_year <- token_lyrics %>%
    group_by(Year) %>%
    summarise(total_words = n(),
              unique_words = n_distinct(words),
              ratio = unique_words / total_words)
...
title_repetition <- function(song_df) {
# initializes empty data frame with columns of Title and Repeats to hold results
  results <- data.frame(Title = character(nrow(song_df)), Repeats = numeric(nrow(song_df)))
# for each row in inputted data frame:
  for (i in 1:nrow(song_df)) { 
    ...
    title_count <- sum(gregexpr(title, lyrics)[[1]] > 0) # sum number of times the title is in the lyrics

    results[i, "Title"] <- title 
    results[i, "Repeats"] <- title_count
  }
  
  mean_repeats <-mean(results$Repeats) # find mean number of repeats 
  cat("Mean number of title repeats in lyrics:", mean_repeats)
   
  return(results)
}

# title_repetition(TopCountry)

title_repetition_2 <- function(song_df) {
  # Initialize empty data frame with columns of Title and Repeats to hold results
  ...
  for (i in 1:nrow(song_df)) { 
    ...
    # Add repeats count to the existing dataframe
    song_df$Repeats[i] <- title_count
  }
  
  # Find mean number of repeats 
  mean_repeats <- mean(song_df$Repeats)
  cat("Mean number of title repeats in lyrics:", mean_repeats)
  
  return(song_df)
}

title_repetition_df <- title_repetition_2(TopCountry)

```
```{r}
library(ggplot2)
ggplot(title_repetition_df, aes(x = Year, y = Repeats)) +
  geom_point() +
