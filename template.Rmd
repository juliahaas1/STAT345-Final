...
## Section Two Tessa
```{r student2}
# TESSA - TITLE REPETITION FUNCTION WITH GRAPH INCLUDED
...
# Purpose: To create a graph of the average title repeats over time.
# The function inputs: 
# The function output: 
graph_title_repetition <- function(title_repetition_df) {
  
  #create table with average title repeats within each song per year
  ave_title_repeats_by_year<- title_repetition_df %>%
  group_by(Year) %>%
  summarize(Year, AveTitleRepeats= mean(repeats)) %>%
  unique()
...
graph_title_repetition(title_repetition_df_TopCountry)
