...
#how well does the linear model explain the trend in the plots?
res<-lm(repeats~Year, title_repetition_df_TopCountry)
summary(res)
#from the summary data, the line is a poor representation of the relationship in the data.
...
TopCountry <- TopCountry[!duplicated(TopCountry$Title), ] # need to fix this
...
unique_words_ratio <- function(song_lyrics) {
  ...
  # find total number of words, then number of unique words  
...
# overall_unique_words_ratio <-unique_words_ratio(TopCountry)
...
unique_words_ratio_by_year <- function(song_df) {
...
unique_words_ratio_by_year <- unique_words_ratio_by_year(TopCountry)
# low ratio means songs are more repetitive

# create graph showing how the unqiue ratio has changed over time:
unique_ratio_graph_by_year <-
  ggplot(unique_words_ratio_by_year, aes(x = Year, y = ratio)) +
  geom_line() +
...
title_repeats_graph <-ggplot(title_repetition_df_TopCountry, aes(x = Year, y = repeats)) +
  geom_point(position = position_jitter(width = 0.1), size = 0.75) +
  geom_smooth(method = "lm",se = FALSE) +
  labs(title = "Average Title Repeats Over Time",
       x = "Year",
       y = "Mean Title Repeats") +
  theme_minimal() +
  # scale_y_continuous(limits = c(0, NA), breaks = seq(0, max(title_repetition_df_TopCountry$Repeats), by = 5)) +
  theme(plot.title = element_text(hjust = 0.5)) 
